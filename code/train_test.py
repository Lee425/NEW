...
class NEW:
    ...
    def computerCost(self, X, y, lam, theta):
        m = y.shape[0]
        C = X.dot(theta) - y
        J_cost = np.sqrt(((C.T.dot(C)) + lam * (theta.T.dot(theta))) / (2 * m))
        return J_cost

    def gradientDescent(self, X, y, lam, theta, alpha, epsilon):
        J_history = []
        iter = 0
        error = 0
        lt = []
        while True and iter < 1e6:
            J_cost = self.computerCost(X, y, lam, theta)
            theta = theta * (1 - alpha * lam) - (alpha * (X.T.dot(X.dot(theta) - y)))
            J_history.append(J_cost)
            if np.abs(J_cost - error) < epsilon:
                break
            else:
                error = J_cost
            iter += 1
    ...
    def training(self, train_x):
        ...
        w = train_x[:, -1]
        J_history, theta, lt = self.gradientDescent(x, w, self.args.lam, init_theta, self.args.learning_rate, self.args.epsilon)
        self.theta = theta
